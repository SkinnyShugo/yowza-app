...
@section('main_content')
    <div class="mt-6 flex flex-col items-center justify-between space-y-2 text-center sm:flex-row sm:space-y-0 sm:text-left">
        <div>
            ...
            <p class="mt-1 hidden sm:block"><a href="{{ route('admin.questions.index', ['prefix' => 'admin']) }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">All</a> | <a href="{{ route('admin.questions.index', ['prefix' => 'admin']) }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Trash</a></p>
        </div>
        <a href="{{ route('admin.questions.create', ['prefix' => 'admin']) }}" class="btn space-x-2 bg-primary font-medium text-white shadow-lg shadow-primary/50 hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:shadow-accent/50 dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
...
{{--                            </th>--}}
                        ...
                        @forelse($questions as $question)
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500" data-entry-id="{{ $question->id }}">
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">{{ $question->id }}</td>

                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <p class="w-48 overflow-hidden text-ellipsis text-xs+">
                                        <span class="badge badge-info">{{ strip_tags($question->question) }}</span>
                                    </p>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">

                                        @if($question->question_image)<a href="{{ Storage::url($question->question_image) }}" target="_blank">
                                        <div class="avatar flex size-10"> <img src="{{  Storage::url($question->question_image) }}"/></div>
                                        </a>@endif
                                            ...
                                            <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                <ul>
                                                    @if( request('show_deleted') == 1 )
                                                        ...
                                                        <li>
                                                            <form action="{{ route('admin.questions.restore', ['prefix' => 'admin', 'question' => $question->id]) }}" method="POST" onsubmit="return confirm('Are you sure ?');" style="display: inline-block;">
                                                                @csrf
                                                        ...
                                                        <li>
                                                            <form action="{{ route('admin.questions.perma_del', ['prefix' => 'admin', 'question' => $question->id]) }}" method="POST" onsubmit="return confirm('Are you sure ?');" style="display: inline-block;">
                                                                @csrf
                                                                @method('delete')
                                                                <button type="submit" class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Delete</button></form>
                                                        </li>
                                                    @else
                                                        <li>
                                                            <a href="{{ route('admin.questions.edit',  ['prefix' => 'admin', 'question' => $question->id]) }}" class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Edit</a>
                                                        </li>
                                                        <li>
                                                            <form action="{{ route('admin.questions.destroy', ['prefix' => 'admin', 'question' => $question->id]) }}" method="POST" onsubmit="return confirm('Are you sure ?');" style="display: inline-block;">
                                                                @csrf
                                                                @method('delete')
