...
@section('main_content')
    <div class="mt-6 flex flex-col items-center justify-between space-y-2 text-center sm:flex-row sm:space-y-0 sm:text-left">
        <div>
            ...
            <p class="mt-1 hidden sm:block"><a href="{{ route('admin.lessons.index', ['prefix' => 'admin']) }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">All</a> | <a href="{{ route('admin.lessons.index', ['prefix' => 'admin']) }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Trash</a></p>
        </div>
        <a href="{{ route('admin.lessons.create', ['prefix' => 'admin']) }}" class="btn space-x-2 bg-primary font-medium text-white shadow-lg shadow-primary/50 hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:shadow-accent/50 dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
            ...
            <div class="card mt-3">
                <div class="is-scrollbar-hidden min-w-full overflow-x-auto">
                    <table class="is-hoverable w-full text-left">
                        ...
                        @forelse($lessons as $lesson)
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500" data-entry-id="{{ $lesson->id }}">

                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">{{ $lesson->id }}</td>
                                ...
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    ...
                                    <div class="badge space-x-2.5 text-xs+ text-success">
                                        <div class="size-2 rounded-full bg-current"></div>
                                        <span> {{ $lesson->published ? 'Active' : 'InActive' }}</span>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div x-data="usePopper({placement:'bottom-end',offset:4})" @click.outside="isShowPopper &amp;&amp; (isShowPopper = false)" class="inline-flex">
                                        ...
                                        <div x-ref="popperRoot" class="popper-root" :class="isShowPopper &amp;&amp; 'show'" style="position: fixed; inset: 0px 0px auto auto; margin: 0px; transform: translate(-151px, 281px);" data-popper-placement="bottom-end">
                                            <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                <ul>
                                                    ...
                                                    @else
                                                    <li>
                                                        <a href="{{ route('admin.lessons.edit',  ['prefix' => 'admin', 'lesson' => $lesson->id]) }}" class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Edit</a>
                                                    </li>
                                                        <li>
                                                            <form action="{{ route('admin.lessons.destroy', ['prefix' => 'admin', 'lesson' => $lesson->id]) }}" method="POST" onsubmit="return confirm('Are you sure ?');" style="display: inline-block;">
                                                                @csrf
                                                                @method('delete')
                ...
                <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                    ...
                    <div class="pagination">
                        {{ $lessons->appends(request()->query())->links('pagination::tailwind') }}
                    </div>

                    <div class="text-xs+">
                        Showing {{ $lessons->firstItem() }} to {{ $lessons->lastItem() }} of {{ $lessons->total() }} entries
                    </div>
                </div>
