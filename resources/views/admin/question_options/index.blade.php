...
@section('main_content')
    <div class="mt-6 flex flex-col items-center justify-between space-y-2 text-center sm:flex-row sm:space-y-0 sm:text-left">
        <div>
            ...
            <p class="mt-1 hidden sm:block"><a href="{{ route('admin.question_options.index', ['prefix' => 'admin']) }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">All</a> | <a href="{{ route('admin.question_options.index', ['prefix' => 'admin']) }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Trash</a></p>
        </div>
        <a href="{{ route('admin.question_options.create', ['prefix' => 'admin']) }}" class="btn space-x-2 bg-primary font-medium text-white shadow-lg shadow-primary/50 hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:shadow-accent/50 dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
            ...
            <span> Create a new question option</span>
        </a>
    </div>
        ...
        <div class="col-span-12">
            <div class="flex items-center justify-between">
                <h2 class="text-base font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100">
                    Question Option(Quiz) Management
                </h2>
                <div class="flex">
            ...
            <div class="card mt-3">
                <p class="is-scrollbar-hidden min-w-full overflow-x-auto">
                    <table class="is-hoverable w-full text-left">
                        ...
                        @forelse($question_options as $question_option)
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500" data-entry-id=""{{ $question_option->id }}">
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">{{ $question_option->id }}</td>

                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <p class="w-48 overflow-hidden text-ellipsis text-xs+">
                                        <span class="badge badge-info">{{ $question_option->question->question ?? '' }}</span>
                                    </p>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <p class="w-48 overflow-hidden text-ellipsis text-xs+">
                                        {{ $question_option->correct == 1 ? 'true' : 'false' }}
                                    </p>
                                        ...
                                        <div x-ref="popperRoot" class="popper-root" :class="isShowPopper &amp;&amp; 'show'" style="position: fixed; inset: 0px 0px auto auto; margin: 0px; transform: translate(-151px, 281px);" data-popper-placement="bottom-end">
                                            <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                <ul>
                                                    ...
                                                    @else
                                                        <li>
                                                            <a href="{{ route('admin.questions.edit',  ['prefix' => 'admin', 'lesson' => $question->id]) }}" class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Edit</a>
                                                        </li>
                                                        <li>
                                                            <form action="{{ route('admin.questions.destroy', ['prefix' => 'admin', 'lesson' => $question->id]) }}" method="POST" onsubmit="return confirm('Are you sure ?');" style="display: inline-block;">
                                                                @csrf
                                                                @method('delete')
                ...
                <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                    ...
                    <div class="pagination">
                        {{ $question_options->appends(request()->query())->links('pagination::tailwind') }}
                    </div>

                    <div class="text-xs+">
                        Showing {{ $question_options->firstItem() }} to {{ $question_options->lastItem() }} of {{ $question_options->total() }} entries
                    </div>
                </div>
